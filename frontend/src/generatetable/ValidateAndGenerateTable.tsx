...
const validateProps = (props: GenerateTableProps): string[] => {
    ...
    const booleanProps: { key: string; value: any }[] = [
      ...
      { key: "isCustomHeaderVisible", value: isCustomHeaderVisible },
    ];
    ...
    //   }
    // }
    if(isCustomHeaderVisible){
        if(!customHeader ){
            errors.push("Invalid prop: 'customHeader' is required and must be a function when 'isCustomHeaderVisible' is true.");
        }
    }
    ...
    const hasFilter = DataColumns.some((column) => column.filter);
    if (hasFilter && !filterType) {
    errors.push("Invalid prop: specify filter type as simple or  advanced when filtering is allowed in a column ");
  }

  // Validate onRowSelection arity
  if (onRowSelection !== undefined) {
    if (typeof onRowSelection !== "function") {
      errors.push("Invalid prop: 'onRowSelection' must be a function.");
    } else if (onRowSelection.length < 2) {
      errors.push("Invalid prop: 'onRowSelection' must accept two arguments: (allSelectedRows, currentRow).");
    }
  }
